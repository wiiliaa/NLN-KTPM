...
export class ProductsService {
    ...
    async findByName(name: string) {
        const found = await this.productRepository.createQueryBuilder("Product").where("Product.name like :name", {name : `%${name}%`}).getMany();
    ...
    async create(createProductDto : CreateProductDto): Promise<Product>{
        let {name,price, weight, description} = createProductDto;
